...
class Game(models.Model):
    ...
    def flood_reveal(self, start_key):
        queue = deque([start_key])

        while queue:
            key = queue.popleft()
            coords = self.get_coordinates_from_key(key)
            if not coords:
                continue

            col, row = coords
            cell = self.board.filter(column=col, row=row).first()
    ...
    def get_coordinates_from_key(self, key):
        if key is None:
            return None
        try:
            return self.key_map.get(str(int(key)))
    ...
    def checkWon(self):
        ...
        if remaining == 0:
            ...
            # Mark all mines as bpoomb safely
